import { d as fe, v as w, u as e } from './db-C6eh-v9M.js';
import { eq, and } from 'drizzle-orm';
import { readFile } from 'fs/promises';
import { existsSync } from 'fs';
import b from 'path';
import j from 'archiver';
import { a as C, c as J, l as m } from './auth-CFRsUa-j.js';
...
const B=async({cookies:v})=>{const g=v.get("session_token");if(!g)return new Response(JSON.stringify({success:false,error:"Unauthorized"}),{status:401,headers:{"Content-Type":"application/json"}});const i=await C(g);if(!i.success||!i.user)return new Response(JSON.stringify({success:false,error:"Unauthorized"}),{status:401,headers:{"Content-Type":"application/json"}});if(!await J(i.user.id,"training.export"))return new Response(JSON.stringify({success:false,error:"Only the System Owner can export training data"}),{status:403,headers:{"Content-Type":"application/json"}});try{const r=await fe.select({id:w.id,userId:w.userId,speakerName:e.fullName,audioPath:w.audioPath,verifiedTranscript:w.verifiedTranscript,rawTranscript:w.rawTranscript}).from(w).innerJoin(e,eq(w.userId,e.id)).where(and(eq(w.isUsable,!0),eq(w.isReviewed,!0)));if(r.length===0)return new Response(JSON.stringify({success:!1,error:"No reviewed training samples available for export"}),{status:404,headers:{"Content-Type":"application/json"}});const a={};for(const p of r){const o=`speaker_${p.userId}`;a[o]||(a[o]=[]),a[o].push(p);}const k=await new Promise(async(p,o)=>{const T=[],t=j("zip",{zlib:{level:6}});t.on("data",e=>{T.push(e);}),t.on("end",()=>{p(Buffer.concat(T));}),t.on("error",e=>{o(e);});for(const[e,u]of Object.entries(a)){let c="",d="",y="";for(const n of u){const l=`${e}_${n.id}`,S=n.verifiedTranscript||n.rawTranscript||"";if(c+=`${l} ${S}
`,d+=`${l} ${e}/audio/${n.id}.wav
`,y+=`${l} ${e}
`,n.audioPath){const $=b.join(process.cwd(),n.audioPath);if(existsSync($))try{const m=await readFile($);t.append(m,{name:`${e}/audio/${n.id}.wav`});}catch(m){console.error("Failed to read audio file:",m);}}}t.append(c,{name:`${e}/text.txt`}),t.append(d,{name:`${e}/wav.scp`}),t.append(y,{name:`${e}/utt2spk`});}let w="";for(const[e,u]of Object.entries(a)){const c=u.map(d=>`${e}_${d.id}`).join(" ");w+=`${e} ${c}
