...
const provider = new GroqPolishProvider();
...
export const POST: RequestHandler = async ({ request, url }) => {
  ...
  try {
    ...
    const polished = await provider.polish(
      { report, template: template as string | undefined, modality, bodyRegion },
      request.signal
    );
    const warnings = checkClinicalSafety(report, polished);
    if (warnings.length) {
      return response({
        success: false,
        accepted: false,
