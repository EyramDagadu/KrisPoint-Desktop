...
`).trim(),e}async smartRefine(s,e,t,o,n=null){if(!this.isAvailable)throw new Error("Ollama is not running. Please start Ollama and try again.");const a=this.stripHtml(s),l=this.stripHtml(e);return console.log("🧹 Stripped HTML from content:",{originalLength:s.length,cleanLength:a.length,hadHtml:s.includes("<")}),this.analyzeCompleteness(a).isComplete?this.polishReport(a,n):this.generateFullReport(a,l,t,n)}async generateFullReport(s,e,t="General",o=null){if(!this.isAvailable)throw new Error("Ollama is not running.");const n=`You are a professional radiologist report writer. Your task is ONLY to rewrite findings into a structured radiology report.
...
CRITICAL: Output the report structure EXACTLY as shown above. Do not add any extra text before or after.`;return this.callOllama(n,o)}async polishReport(s,e=null){if(!this.isAvailable)throw new Error("Ollama is not running.");const t=`You are a professional radiologist editor. Your task is ONLY to improve the language and clarity of a radiology report.
...
CRITICAL: Respond with ONLY the impression text. No labels, no extra text. Start directly with the impression.`;return this.callOllama(n)}async callOllama(s,e=null){try{let t="";const o={model:this.model,prompt:s,stream:!0,temperature:.3},n=this.isTauriRuntime();console.log("🚀 Ollama request:",{isTauri:n,url:`${this.baseUrl}/api/generate`,model:this.model,fetchType:n?"Tauri backend proxy (no streaming)":"Browser fetch (streaming)",env:f?.TAURI_PLATFORM||"browser"});const a=await this.makeRequest(`${this.baseUrl}/api/generate`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(console.log("📥 Ollama response:",{ok:a.ok,status:a.status,statusText:a.statusText,headers:a.headers}),!a.ok){const l=await a.text();throw console.error("❌ Ollama error response:",l),new Error(`Ollama API error: ${a.status} ${a.statusText}`)}if(n){console.log("🖥️ Tauri mode: Getting full response (no streaming)");const c=(await a.text()).split(`
`).filter(i=>i.trim());for(const i of c)try{const r=JSON.parse(i);r.response&&(t+=r.response)}catch{}e&&t&&(console.log("✅ Full response ready, length:",t.length),e(t))}else{const l=a.body.getReader(),c=new TextDecoder;let i="";for(;;){const{done:r,value:g}=await l.read();if(r)break;const h=c.decode(g,{stream:!0});i+=h;const d=i.split(`
`);i=d.pop()||"";for(const u of d)if(u.trim())try{const p=JSON.parse(u);p.response&&(t+=p.response,console.log("🔄 Streaming token:",p.response),e&&e(t))}catch(p){console.warn("JSON parse error:",p.message,"Line:",u)}}if(i.trim())try{const r=JSON.parse(i);r.response&&(t+=r.response,e&&e(t))}catch{}}return console.log("✅ Ollama generation complete, total length:",t.length),t.trim()}catch(t){throw console.error("❌ Ollama API error:",{message:t.message,stack:t.stack,name:t.name,error:t}),t}}analyzeCompleteness(s){const e=/COMPARISON:/i.test(s),t=/TECHNIQUE:/i.test(s),o=/FINDINGS:/i.test(s),n=/IMPRESSION:/i.test(s),a=s.replace(/\s+/g," ").length,l=a>300;return{isComplete:t&&o&&n&&l,sections:{hasComparison:e,hasTechnique:t,hasFindings:o,hasImpression:n},contentLength:a}}}const I=new T;export{I as o};
